...
def search_multiple_keywords(
	keywords: list, courses: list, fuzzy: bool = False, verbose=False
) -> list[str]:
	"""
...
if __name__ == "__main__":
	...
	parser.add_argument(
		"keyword", type=str, nargs="?", help="The keyword to search for in course data."
	)
	parser.add_argument(
	...
	if args.interactive:
		while True:
			if "\n" in keyword:
				...
				output = search_multiple_keywords(
					keywords, courses, fuzzy, verbose=False
				)
				print(output)
				keyword = ""
				continue
			if keyword:
				...
				keyword = console.input(
					"[green]Enter a keyword to search for in course data: [/green]"
				)
			if keyword == "exit":
				break
			if keyword == "fuzzy":
